### Mapiranje za tabelu 21.1.1 iz .sjz.rs baze: ----
### ---
...
### --
### Age of use = age_cat_pocetak_koriscenja
### Starosne grupe po 5 godina širine
### Total = Zbir svega 
### ---
### 21.1.1 Primary drug by age at first use –  All treatment entrants  (Never previously treated + Previously treated + Not known/Missing)
### ---
### Sracunaj vrednosti za  TDI tabelu 21.1.1 ----
...
baza_working <- baza |> 
  filter(age_cat_pocetak_koriscenja == "0-14")
...
baza_working <- baza |> 
  filter(age_cat_pocetak_koriscenja == "15-19")
...
baza_working <- baza |> 
  filter(age_cat_pocetak_koriscenja == "20-24")
...
baza_working <- baza |> 
  filter(age_cat_pocetak_koriscenja == "25-29")
...
baza_working <- baza |> 
  filter(age_cat_pocetak_koriscenja == "30-34")
...
baza_working <- baza |> 
  filter(age_cat_pocetak_koriscenja == "35-39")
...
baza_working <- baza |> 
  filter(age_cat_pocetak_koriscenja == "40-44")
...
baza_working <- baza |>   
  filter(age_cat_pocetak_koriscenja == "45-49")
...
baza_working <- baza |> 
  filter(age_cat_pocetak_koriscenja == "50-54")
...
baza_working <- baza |> 
  filter(age_cat_pocetak_koriscenja == "55-59")
...
baza_working <- baza |> 
  filter(age_cat_pocetak_koriscenja == "60-64")
...
baza_working <- baza |>  
  filter(age_cat_pocetak_koriscenja == "65+")
...
baza_working <- baza |> 
  filter(is.na(age_cat_pocetak_koriscenja))
...
# Create sheet and file with Table_21.1.1  ----------------------------------

tabela_21.1.1 <- data.frame(
  ...
  under15 = c("<15", c1r1, c1r2, c1r3, c1r4, c1r5, c1r6, c1r7, c1r8, c1r9, c1r10, c1r11, c1r12, c1r13, c1r14, c1r15, c1r16, c1r17, c1r18, c1r19, c1r20, c1r21, c1r22, c1r23, c1r24, c1r25, c1r26, c1r27, c1r28, c1r29, c1r30, c1r31, c1r32, c1r33),
...
write.xlsx(tabela_21.1.1, file="data/izlazniPodaci.xlsx", sheetName="tabela_21.1.1", append=TRUE, col.names=FALSE, row.names=FALSE)
