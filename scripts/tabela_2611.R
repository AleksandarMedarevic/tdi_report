### Mapiranje za tabelu 26.1.1 iz .sjz.rs baze: ----
### ---
### HIV testing  = Testiranje_na_HIV
...
### Total = Zbir svega 
### ---
### 26.1.1 HIV testing by injecting behaviour -  All treatment entrants  (Never previously treated + Previously treated + Not known/Missing)
### ---
### Sracunaj vrednosti za  TDI tabelu 26.1.1 ----
...
c1r1 <- baza_working |> 
  filter(Testiranje_na_HIV == "1 - Nije testiran/a") |> 
  nrow()

c1r2 <- baza_working |> 
  filter(Testiranje_na_HIV == "2 - Da, u poslednjih 12 meseci" |
           Testiranje_na_HIV == "3 - Da, ali ne u poslednjih 12 meseci") |> 
  nrow()

c1r3 <- baza_working |> 
  filter(Testiranje_na_HIV == "3 - Da, ali ne u poslednjih 12 meseci") |> 
  nrow()
...
c1r5 <- baza_working |> 
  filter(Testiranje_na_HIV == "4 - Ne želi da odgovori") |> 
  nrow()

c1r6 <- baza_working |> 
  filter(Testiranje_na_HIV == "0 - Nepoznat podatak") |>  
  nrow()
...
# Create sheet and file with Table_26.1.1  ----------------------------------

tabela_26.1.1 <- data.frame(
...
write.xlsx(tabela_26.1.1, file="data/izlazniPodaci.xlsx", sheetName="tabela_26.1.1", append=TRUE, col.names=FALSE, row.names=FALSE)
